...
public class FurnitureOBJImporter : MonoBehaviour
{
    ...
    private IEnumerator Load(string path)
    {
        ...
        objname = System.IO.Path.GetFileName(uri.LocalPath).Split('.')[0];

        Debug.Log("Loading model from " + path + " name " + objname);
        WWW loader = new WWW(path);
        yield return loader;
        Debug.Log("Got model data");
    ...
    private void Build()
    {
        ...
        Debug.Log("What game object???  " + gameObject.name);
        for (int i = 0; i < buffer.numObjects; i++)
        {
            GameObject meshParent = new GameObject();
            //meshParent.AddComponent<PlaceModel>();

            GameObject target = new GameObject();
            target.transform.localScale = new Vector3(1.0F, 1.0F, 1.0F);
            target.transform.parent = meshParent.transform;
            target.AddComponent<BoxCollider>();

            GameObject meshObj = new GameObject();
            meshObj.transform.parent = meshParent.transform;
            meshObj.AddComponent<MeshFilter>();
            meshObj.AddComponent<MeshRenderer>();
            meshObj.AddComponent<TapToPlaceParent>();
            meshObj.transform.position = Camera.main.ViewportToWorldPoint(new Vector3(0.5f, -0.5f, 2.0f));
            meshObj.transform.localScale = new Vector3(0.7f, 0.7f, 0.7f);
            ms[i] = meshObj;

            meshParent.transform.parent = gameObject.transform;
            furnitureList.Add(meshParent);
        }
    ...
    {
        foreach (GameObject entry in furnitureList)
        {
            Debug.Log("NOW deleting...");
            Transform parent = entry.transform;
